...
function display(item) {
    ...
    output = '<div class="display-container">'
        ...
        + '<div class="material">';
    let add = [];
    add = sumValuesBetween5(1, 90);
    output += '<div class="items" title="摩拉">'
        + '<img src="其他/摩拉.png" style=" background-color: #53799e;">'
        + '<div class="name" style="font-size: 14px">'
        + add[0].sum + 'k</div></div>'
        + '<div class="items" title="魔礦">'
        + '<img src="其他/魔礦.png" style=" background-color: #53799e;">'
        + '<div class="name" style="font-size: 14px">'
        + add[1].sum + '</div></div>';
...
function sumValuesBetween5(start, end) {
    const count = [];

    for (const key in five) {
        const array = five[key];
        let sum = 0;
        for (let j = 0; j < array.length; j++) {
            sum += array[j];
        }
        count.push({ arrayName: key, sum: sum });
    }
    return count.length > 0 ? count : [{ arrayName: null, sum: 0 }];
}
